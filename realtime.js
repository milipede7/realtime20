(function() {
	var charWidth = 6;
	var charHeight = 10;
	var likelihoodOfReplacingWord = 5;
	var likelihoodOfChangingExistingText = 0.05;
	...
	function initElement(element) {
		...
		img.src = element.getAttribute('data-letter');
	}
	...
	function render(element, image, prev) {
		if (element.hasAttribute('data-letter-aspect-ratio')) {
			var aspect = parseFloat(element.getAttribute('data-letter-aspect-ratio'));
			element.style.height = element.clientWidth * aspect + 'px';
		}
		var text;
		var words = element.hasAttribute('data-letter-words') ? element.getAttribute('data-letter-words').split(' ') : [];
		var letters = element.hasAttribute('data-letter-letters') ? element.getAttribute('data-letter-letters') : '0101010101_';
	...
	document.addEventListener('DOMContentLoaded', function() {
		var elements = document.querySelectorAll('[data-letter]');
